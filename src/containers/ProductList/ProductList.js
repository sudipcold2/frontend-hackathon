                ...
                <div className="row">
                    {paginationPipe(this.props.products, this.state).map(product =>{
                        ...
                        return (<div className={classes}>
                            <Product key={product.bookID} product={product} />
                        </div>)
                    })}
                </div>
...
const mapStateToProps = state => {
    console.log("dfsdgfsdbgibsdkgbisdgbisd " + state);
    //const brands = state.brandFilter;
    const orderBy = state.orderBy;
    const filterByBrandArr = state.books;
    const filterByOrderArr = orderByFilter(filterByBrandArr, orderBy);

    return {products: filterByOrderArr};
    // return {products: state.books}
};

export default connect(mapStateToProps, {fetchBooks: fetchBooks})(ProductList);
